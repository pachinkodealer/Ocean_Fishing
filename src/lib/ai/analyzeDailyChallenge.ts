...
import { z } from 'zod'
import { DAILY_SYSTEM_PROMPT, buildDailyUserPrompt, RETRY_PROMPT, type KlineRow } from './prompts'
...
export async function analyzeDailyChallenge(
  ...
  const userPrompt = buildDailyUserPrompt(ticker, interval, klines)

  let rawText = await callClaude(userPrompt)

  let parsed: unknown
  try {
    parsed = JSON.parse(rawText)
  } catch {
    rawText = await callClaude(RETRY_PROMPT)
    try {
      parsed = JSON.parse(rawText)
    } catch {
